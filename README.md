...
- 支持在详情页确认后永久删除码池。`DELETE /api/manage/pools/:id` 校验空间归属，在同一事务中删除全部兑换码和码池；重复删除返回 404，前端按已删除处理。原领码 Key 与链接失效，同名可重新创建，但 ID 和 Key 不复用。删除不会收回已经发出的兑换码，浏览器保存的历史仍可查看、复制，但无法继续提交使用标记。
- 每次最多导入 500 条，每条最长 100 个 Unicode 码点；去首尾空白、忽略空行、区分大小写。合法行导入，重复/超长行跳过并返回原始行号及原因。
- 管理端集中展示库存统计、导入和分享操作；领码 Key 默认折叠，改名、停止/恢复和删除收在「更多」中。明细按创建时间倒序，默认每页 20 条，可切换 50 条，并按「全部、未领取、未使用、已使用」筛选；未使用指已领取但尚未标记使用，已使用保留首次标记时间；切换条数或筛选时回到第一页。接口未传 `pageSize` 时保持每页 50 条。
- 未领取的兑换码支持逐条确认删除，删除后刷新库存及分页。`DELETE /api/manage/pools/:id/codes/:codeId` 校验空间、码池和兑换码归属，并在删除语句中检查未领取状态；已领取（包括未使用及已使用）返回 409，防止与领取并发时误删。重复删除返回 404，前端按已删除处理。旧接口筛选值 `claimed` 仍兼容已领取的所有兑换码。
- 「未领取」页签支持逐条勾选、全选当前页及批量确认删除；选择仅属于当前页，翻页、切换筛选/条数或刷新后清空。`DELETE /api/manage/pools/:id/codes` 接收 1–50 个兑换码 ID，以单条语句删除本池中仍未领取的记录，返回删除和跳过数量；已领取或不存在的记录自动跳过。
