...
| `npm run deploy` | Apply remote D1 migrations and publish the existing build (see the first-upgrade procedure below) |
...
For existing installations upgrading from before `0002_pool_counters`, follow the maintenance procedure below instead of step 4. For subsequent releases, update your checkout and repeat step 4. Upload the secret again only when changing it. Wrangler uses the configuration generated by the Vite build automatically; no separate deployment configuration or configuration-generation script is needed.
...
### First upgrade to application-maintained counters

Fresh installations can use the normal deployment flow. Existing installations must use a maintenance window: old Worker versions do not update the new counters. Disable automatic deployments for this upgrade so they cannot bypass maintenance. Build and validate first, then publish the maintenance entry point:
...
This deploys to the same Worker and routes from `wrangler.jsonc`, using `dist/client` for static assets. It does not apply database migrations. Confirm `/api/config` and all other API entry points return 503, stop external writers, and use Workers logs to confirm the old version's in-flight requests have completed. Publishing maintenance does not cancel in-flight requests. Only then run:
...
Run each command only after the preceding step succeeds. Migration `0002_pool_counters` backfills all counters with one grouped aggregate. If migration or validation fails, keep maintenance active; do not publish the application. If counters need repair while all writers remain paused:

```bash
npm run db:counters:rebuild -- --remote --writes-paused
```

The rebuild command performs a validation afterward and exits nonzero on mismatch. `--writes-paused` confirms an operational precondition; it does not pause requests itself. Resume external writers and automatic deployments only after the new application is healthy.

Use the same maintenance window for rollback. If an older version resumes writes, counters become stale; rebuild and verify them with writers paused before deploying a counter-aware version again. Never use the ordinary migration-and-deploy command to skip this first-upgrade procedure.
